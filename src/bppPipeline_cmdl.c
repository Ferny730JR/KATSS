/*
  ...
  gengetopt -i bppPipeline.ggo --file-name=bppPipeline_cmdl --include-getopt --default-optional --unamed-opts --func-name=bppPipeline_cmdline_parser --arg-struct-name=bppPipeline_args_info
...
const char *bppPipeline_args_info_description = "The Base-Pair Probability Pipeline is a C program designed to analyze RNA\nsequences and identify enriched        base-pair interactions in protein-bound\nRNA. It follows a structured pipeline that includes RNA folding, k-mer\ncounting, and enrichment analysis, producing a CSV file with ranked k-mers\nbased on their likelihood of being        bound or unbound.";
...
init_help_array(void)
{
  ...
  bppPipeline_args_info_help[8] = bppPipeline_args_info_detailed_help[12];
  bppPipeline_args_info_help[9] = 0; 
  
}

const char *bppPipeline_args_info_help[10];
...
void init_args_info(struct bppPipeline_args_info *args_info)
{
  ...
  args_info->noBin_help = bppPipeline_args_info_detailed_help[11] ;
  args_info->frq_help = bppPipeline_args_info_detailed_help[12] ;
...
bppPipeline_cmdline_parser_dump(FILE *outfile, struct bppPipeline_args_info *args_info)
{
  ...
  if (args_info->noBin_given)
    write_into_file(outfile, "noBin", 0, 0 );
  ...
  while (1)
    {
      ...
      c = custom_getopt_long (argc, argv, "hVi:b:o:k:", long_options, &option_index);
...
.  */
          if (strcmp (long_options[option_index].name, "noBin") == 0)
          {
          
          
            if (update_arg((void *)&(args_info->noBin_flag), 0, &(args_info->noBin_given),
                &(local_args_info.noBin_given), optarg, 0, 0, ARG_FLAG,
                check_ambiguity, override, 1, 0, "noBin", '-',
                additional_error))
              goto failure;
